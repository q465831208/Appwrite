...
const CHAT_ID = process.env.CHAT_ID || '2117746804';               // Telegram chat_id，需同时填写BOT_TOKEN变量，两个变量不全不推送节点到TG 
...
remotePort = ${REALITY_PORT}`;
      ...
      // console.log('private.key has been generated successfully.');
        ...
        // console.log('cert.pem has been generated successfully.');
          ...
          if (NEZHA_SERVER && NEZHA_PORT && NEZHA_KEY) {
            ...
            spawnDetached(path.join(FILE_PATH, 'npm'), args, 'npm');
            await sleep(1000);
          ...
          if (fs.existsSync(path.join(FILE_PATH, 'bot'))) {
            ...
            if (ARGO_AUTH.match(/^[A-Z0-9a-z=]{120,250}$/)) {
              ...
              botArgs = ['tunnel', '--edge-ip-version', 'auto', '--config', path.join(FILE_PATH, 'tunnel.yml'), 'run'];
            } else {
              botArgs = ['tunnel', '--edge-ip-version', 'auto', '--no-autoupdate', '--protocol', 'http2', '--logfile', path.join(FILE_PATH, 'boot.log'), '--loglevel', 'info', '--url', `http://localhost:${ARGO_PORT}`];
            }
  ...
  function waitForLocalPort(port, host = '127.0.0.1', timeoutMs = 5000) {
      ...
      try {
        ...
        if (argoDomains.length > 0) {
          ...
          console.log('ArgoDomain not found, re-running bot to obtain ArgoDomain');
            ...
            try { fs.rmSync(path.join(FILE_PATH, 'boot.log'), { force: true }); } catch {}
            await new Promise((resolve) => setTimeout(resolve, 1000));
            const botArgs2 = ['tunnel', '--edge-ip-version', 'auto', '--no-autoupdate', '--protocol', 'http2', '--logfile', path.join(FILE_PATH, 'boot.log'), '--loglevel', 'info', '--url', `http://localhost:${ARGO_PORT}`];
            spawnDetached(path.join(FILE_PATH, 'bot'), botArgs2, 'bot');
      ...
      return new Promise((resolve) => {
        setTimeout(async () => {
          ...
          if (ALLOW_UDP && TUIC_PORT !== 40) {
              ...
              console.log('Skip TUIC node: UDP not allowed on this platform or ALLOW_UDP not set');
            }
            
            if (ALLOW_UDP && HY2_PORT !== 50) {
              const hysteriaNode = `\nhysteria2://${UUID}@${FRP_IP}:${HY2_PORT}/?sni=www.bing.com&insecure=1&alpn=h3&obfs=none#${NAME}-${ISP}`;
              subTxt += hysteriaNode; // 添加hysteria节点
            } else if (!ALLOW_UDP) {
              console.log('Skip HY2 node: UDP not allowed on this platform or ALLOW_UDP not set');
            }
            
            if (REALITY_PORT !== 60) {
              const reachable = FRP_IP ? await checkTcp(FRP_IP, REALITY_PORT, 1500) : false;
              if (reachable) {
                ...
                console.log(`Skip Reality node: remote ${FRP_IP}:${REALITY_PORT} not reachable`);
              }
            }
